...
/// Guest-facing feed of trip/booking updates and suggestions - see
/// schema_notifications.sql and NotificationsService for how each
/// type gets created (some trigger-driven server-side, some
/// generated lazily by this screen on open).
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  // Tracks per-row "opening..." state so a slow tap can't be fired
  // twice while its booking is being fetched.
  String? _openingId;
  ...
  void _load() {
    setState(() {
      _future = withRetry(() async {
        // Generate any newly-due time-based notifications first, so
        // this always shows an up-to-date feed rather than whatever
        // was there the last time the guest happened to open it.
  ...
  Future<void> _openNotification(AppNotification notification) async {
    ...
    try {
      final booking = await _service.getBookingById(notification.bookingId!);
      if (!mounted) return;
      if (booking == null) {
      ...
      // A review-prompt notification means the stay is over and
      // there's nothing left to manage on it - so instead of dropping
      // the guest onto the general BookingDetailScreen, take them
      ...
      if (notification.type == 'review_prompt') {
        ...
        // Straight into the conversation with the host rather than
        // the booking detail screen - that's what the guest tapped
        // the notification to see.
        if (booking.hostId == null) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("That conversation isn't available anymore.")),
          );
        } else {
          await Navigator.of(context).push(
            MaterialPageRoute(
              builder: (_) => ChatScreen(
                ...
                otherPartyId: booking.hostId!,
                otherPartyName: (booking.hostName ?? '').trim().isNotEmpty
                    ? booking.hostName!.trim()
                    : 'Host',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Updates about your trips - confirmations, reminders, and '
